...
Astro is an open-source project that allows developers to create API tests with code and run those tests against their production APIs. It's like Datadog Synthetics, but it lives in your codebase.

## Project Structure

Astro consists of four separate repositories:
...
**Key Features:**
- Discovers `.ts` test files in `__astro__` subdirectories
...
### 3. [astro-test-system](./astro-test-system/)
...
### 4. [astro-plan-executor](./astro-plan-executor/)
...
2. **Create a test file** in a `__astro__` directory:
   ```typescript
   // __astro__/my-test.ts
   import { GET, ApiCheckBuilder, JSON, START, END, Frequency } from "../astro-test-system/src/index";
...
1. **Write Tests**: Create `.ts` files in `__astro__` subdirectories using the test system DSL. Each test specifies its own `endpoint_host` (including port) in the `ApiCheckBuilder` configuration.
2. **Run Locally**: Use `node astro-cli/dist/cli.js run-local` or `npx astro-cli run-local` to run all tests using their configured endpoint hosts
3. **Configure Runner** (coming soon): Use `npx astro-cli configure-runner-host <host>` to set up remote execution
4. **Deploy** (coming soon): Use `npx astro-cli deploy` to deploy tests to a runner
5. **Monitor** (coming soon): Use `npx astro-cli logs <check-name>` to view execution logs
