...
The Astro Test System provides a TypeScript DSL for defining API tests. Tests are written in TypeScript and output JSON test plans that can be executed by the plan executor.
...
Create test files in `__astro__` directories. When executed, they output JSON test plans.
...
```typescript
import { GET, POST, ApiCheckBuilder, JSON, START, END, Frequency, Wait } from "../astro-test-system/src/index";
