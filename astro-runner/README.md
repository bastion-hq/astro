...
The Astro Runner is an orchestration service responsible for scheduling and executing API tests. It ensures tests run according to their configured frequencies (e.g., every 15 minutes, every minute, etc.).
...
```bash
docker run -it -e POSTGRESQL_URL=postgresql://some-db-instance.aws.com astro:runner
```
