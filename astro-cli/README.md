...
The Astro CLI tool allows developers to run API tests locally and manage their test configurations. It scans for `.ts` files in `__astro__` subdirectories and executes them using the test system.

## Features

✅ **Currently Working**:
- Scan and discover test files in `__astro__` subdirectories
...
- Built `astro-test-system` and `astro-plan-executor` projects (see main README)
...
The CLI will:
1. Discover all `.ts` files in `__astro__` directories
...
Found 1 test file(s):
  - /path/to/__astro__/example-check.ts
...
```bash
npx astro-cli configure-runner-host "https://runner-host.com"
```

This saves the runner host configuration to `~/.astro/config.json`.

### Other Commands (Coming Soon)

```bash
# Deploy tests to the configured runner
npx astro-cli deploy

# View logs for a specific check
npx astro-cli logs foo-bar-check

# Execute a check remotely
npx astro-cli execute-remote foo-bar-check
```

## How It Works

1. **Discovery**: The CLI scans the current directory (and subdirectories) for `__astro__` folders containing `.ts` files
...
**"Test system not built"**: Make sure you've built `astro-test-system`:
```bash
cd ../astro-test-system && npm install && npm run build
```

**"Plan executor not built"**: Make sure you've built `astro-plan-executor`:
```bash
