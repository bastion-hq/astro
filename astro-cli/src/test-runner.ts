...
export async function runTestFile(
  ...
  const executorPath = path.join(workspaceRoot, 'astro-plan-executor', 'dist');
  ...
  if (!fs.existsSync(executorPath)) {
    throw new Error(
      'Plan executor not built. Please run: cd astro-plan-executor && npm install && npm run build'
    );
  }
